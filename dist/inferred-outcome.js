...
function chunksFromResult(content) {
...
export function parseTranscript(lines) {
    ...
    for (const line of lines) {
        ...
        if (obj.type === 'user' && Array.isArray(content)) {
            for (const p of content) {
                if (!p || p.type !== 'tool_result' || typeof p.tool_use_id !== 'string')
                    continue;
                ...
                if (chunks.length)
                    searches.push({ toolUseId: p.tool_use_id, query: issued.query, turn: issued.turn, chunks });
            }
        }
    }
 ...
 */
export function distinctiveTokens(content) {
    const out = new Map();
    const add = (tok, w) => {
    ...
    for (const m of content.matchAll(/`([^`]{6,80})`/g))
        add(m[1], 2);
    for (const m of content.matchAll(/"([^"]{8,80})"/g))
        add(m[1], 2);
    ...
    for (let i = 0; i < words.length; i++) {
        ...
        if (w.length < 4)
            continue;
        if (/^[A-Za-z0-9._~-]+\/[A-Za-z0-9._~\/-]+$/.test(w)) {
            add(w, 3);
            continue;
        }
        if (/^[0-9A-Za-z]{16,}$/.test(w) && /\d/.test(w) && /[A-Za-z]/.test(w)) {
            add(w, 3);
            continue;
        }
        if (/^[0-9a-f]{12,}$/i.test(w)) {
            add(w, 3);
            continue;
        }
        ...
        if (/^[A-Z][a-z]{4,}$/.test(w) && !prevEndsSentence)
            add(w, 1);
    }
    return out;
}
export function chunkWasUsed(content, laterOutput) {
    let score = 0;
    for (const [tok, w] of distinctiveTokens(content)) {
        if (laterOutput.includes(tok)) {
            score += w;
            if (score >= HELPFUL_THRESHOLD)
                return true;
        }
    }
    return false;
 ...
 */
export function gradeSearches(parsed, opts = {}) {
    ...
    for (const s of parsed.searches) {
        if (!opts.final && lastTurn - s.turn < minAfter) {
            immature++;
            continue;
        }
        ...
        for (const c of s.chunks)
            (chunkWasUsed(c.content, later) ? helpful : irrelevant).push(c.id);
...
export function readGradedState(dataDir, sessionId) {
    const p = statePath(dataDir, sessionId);
    if (!existsSync(p))
        return new Set();
    try {
        const parsed = JSON.parse(readFileSync(p, 'utf8'));
        return new Set(Array.isArray(parsed.toolUseIds) ? parsed.toolUseIds.filter((x) => typeof x === 'string') : []);
    }
    catch {
        return new Set();
    }
}
export function writeGradedState(dataDir, sessionId, ids) {
 ...
 */
export async function gradeTranscript(config, openStorage, transcriptPath, sessionId, opts = {}) {
    const summary = { graded: 0, helpful: 0, irrelevant: 0, immature: 0, alreadyGraded: 0 };
    ...
    for (const g of fresh) {
        if (storage) {
            if (g.helpful.length)
                await recordRecallOutcome(config, storage, g.helpful, 'helpful', sessionId);
            if (g.irrelevant.length)
                await recordRecallOutcome(config, storage, g.irrelevant, 'irrelevant', sessionId);
        }
        summary.graded++;
    ...
    if (!opts.dryRun)
        writeGradedState(config.dataDir, sessionId, done);
    return summary;
}
//# sourceMappingURL=inferred-outcome.js.map