 ...
 */
export async function rebuildRules(config, storage) {
    const old = await storage.getRules();
    for (const r of old)
        await storage.deleteRule(r.id);
...
async function extractKgTriples(config, storage, state) {
    if (!isLlmAvailable())
        return 0;
    const chunks = await storage.listChunks();
 ...
 */
async function normalizeTaxonomyPass(storage) {
    const chunks = await storage.listChunks();
    let rewritten = 0;
    storage.beginBatch();
    try {
        for (const chunk of chunks) {
            ...
            if (domain === (chunk.domain ?? '') && topic === (chunk.topic ?? ''))
                continue;
 ...
 */
async function writeDiaryDigest(config, storage) {
    const today = new Date().toISOString().split('T')[0];
    try {
        if (listDiaryDates(config.dataDir).includes(today))
            return false;
