 ...
 */
export async function detectContradictions(config, storage, newContent, opts) {
...
async function findCandidatesByEmbedding(storage, embedding, opts) {
    ...
    const all = [...chunks, ...longTerm]
        .filter(c => {
        if (opts?.domain && c.domain && c.domain !== opts.domain)
            return false;
        if (opts?.topic && c.topic && c.topic !== opts.topic)
            return false;
...
async function findCandidatesByKeywords(storage, content, opts) {
...
function heuristicContradictionCheck(newContent, candidates) {
    const result = { found: false, contradictions: [] };
    ...
    for (const existing of candidates) {
        ...
        if (!subjectsOverlap(newSubject, existingSubject))
            continue;
...
function detectValueContradiction(newText, existingText) {
...
async function llmContradictionCheck(config, newContent, candidates) {
...
Return [] if no contradictions found. Return ONLY valid JSON.`, `NEW MEMORY:\n${newContent}\n\nEXISTING MEMORIES:\n${existingList}`, { maxTokens: 300, temperature: 0 });
    ...
    try {
        ...
        for (const item of parsed) {
            if (typeof item.index !== 'number' || !candidates[item.index])
                continue;
            result.found = true;
            result.contradictions.push({
                newContent,
 ...
 */
export async function measureSemanticDrift(config, storage, opts) {
    const report = { driftDetected: false, dimensions: [], warnings: [] };
    ...
    for (const [domain, { old, recent }] of domains) {
        if (old.length < 3 || recent.length < 3)
            continue;
        ...
        if (oldWithEmb.length >= 3 && newWithEmb.length >= 3) {
            ...
            if (drift > 0.3) {
                ...
                report.warnings.push(drift > 0.5
...
const INJECTION_PATTERNS = [
    { pattern: /\b(ignore previous instructions|ignore all instructions|disregard|forget everything)\b/i, reason: 'Prompt injection attempt', severity: 'high' },
    { pattern: /^(system|SYSTEM)\s*:/m, reason: 'System prompt injection marker', severity: 'high' },
    { pattern: /\bIMPORTANT\s*:.*\b(must|always|never|override)\b/i, reason: 'Authority escalation pattern', severity: 'medium' },
    { pattern: /[A-Za-z0-9+/]{100,}={0,2}/, reason: 'Suspicious base64 blob', severity: 'medium' },
    { pattern: /\S{80,}/, reason: 'Extremely long token (possible obfuscation)', severity: 'low' },
    { pattern: /\b(act as|you are now|pretend to be|new persona|new identity)\b/i, reason: 'Identity override attempt', severity: 'high' },
];
/**
 * Check recent memories for poisoning patterns.
 * All heuristic — no LLM needed.
 */
export async function checkMemoryPoisoning(storage, recentChunks) {
    const result = { suspicious: false, flags: [] };
 ...
 */
export async function runGovernanceCheck(config, storage, opts) {
    const [contradictions, drift, poisoning] = await Promise.all([
        opts?.content
            ...
            : Promise.resolve({ found: false, contradictions: [] }),
