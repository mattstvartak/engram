...
// ── Daily → Short-term ───────────────────────────────────────────────
async function processDailyTier(storage, chunks, cfg) {
    let moved = 0;
    const now = Date.now();
...
async function archiveStaleShortTerm(storage, chunks, cfg) {
    let archived = 0;
    const now = Date.now();
...
// ── Long-term → Archive ──────────────────────────────────────────────
async function demoteToArchive(storage, chunks, cfg) {
    let demoted = 0;
    const now = Date.now();
    for (const chunk of chunks) {
...
export function instructionFloor(chunk) {
...
async function decayImportance(storage, chunks) {
    ...
    const floorFor = (chunk) => Math.max(floors[chunk.cognitiveLayer] ?? 0.10, instructionFloor(chunk));
    for (const chunk of chunks) {
        if (chunk.tier === 'archive')
            continue;
