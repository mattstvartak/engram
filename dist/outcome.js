 ...
 */
export async function recordRecallOutcome(config, storage, chunkIds, outcome, conversationId, recentMessages) {
    ...
    for (const id of chunkIds) {
        ...
        if (outcome === 'helpful')
            ...
            importance = Math.max(0.1, importance - 0.1);
        else if (outcome === 'irrelevant')
            importance = Math.max(0.1, importance - 0.05);
