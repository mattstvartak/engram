...
export async function extractTriplesLlmBatch(config, inputs) {
    const out = new Map();
    if (!isLlmAvailable() || inputs.length === 0)
        return out;
    ...
    if (!Array.isArray(parsed))
        return out;
    const allowed = new Set(KG_PREDICATES);
    for (const row of parsed) {
        if (typeof row !== 'object' || row === null)
            continue;
        const { i, s, p, o } = row;
        if (typeof i !== 'number' || i < 0 || i >= inputs.length)
            continue;
        ...
        if (subject.length < 2 || subject.length > 80)
            continue;
        if (object.length < 2 || object.length > 120)
            continue;
        if (object.split(/\s+/).length > 8)
            continue;
        const id = inputs[i].id;
        const list = out.get(id) ?? [];
        const key = `${subject.toLowerCase()}|${predicate}|${object.toLowerCase()}`;
        if (list.some(r => `${r.subject.toLowerCase()}|${r.predicate}|${r.object.toLowerCase()}` === key))
            continue;
...
export async function llmExtractAndPersist(config, storage, chunks) {
    let added = 0;
    for (let i = 0; i < chunks.length; i += LLM_BATCH_SIZE) {
        ...
        for (const [id, results] of extracted) {
            ...
            for (const result of results) {
                try {
                    await addTriple(storage, result.subject, result.predicate, result.object, `auto-extract-llm:${source}`, result.confidence);
                    added++;
                }
