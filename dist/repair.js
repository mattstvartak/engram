...
export function rejoinedSource(source) {
 ...
 */
export function isCutPiece(content) {
    const t = content.trim();
    if (!t)
        return false;
    const opens = (t.match(/\(/g) ?? []).length;
 ...
 */
export function rejoinGroups(chunks, opts = {}) {
 ...
 */
export function dedupePieces(pieces) {
    const kept = [];
    const seen = [];
    for (const p of pieces) {
        ...
        if (!n)
            continue;
        if (seen.some(x => x === n || x.includes(n) || n.includes(x)))
            continue;
        seen.push(n);
        kept.push(p);
    }
...
export function mergeContent(pieces) {
...
export async function repairStore(config, storage, opts = {}) {
    const dryRun = !!opts.dryRun;
    const all = await storage.listChunks();
    const byId = new Map(all.map(c => [c.id, c]));
    const report = { groupsConsidered: 0, groupsRejoined: 0, piecesRemoved: 0, floored: 0, summariesTrimmed: 0, summariesDeleted: 0, dryRun };
    for (const g of rejoinGroups(all)) {
        report.groupsConsidered++;
        const pieces = g.ids.map(id => byId.get(id)).filter((c) => !!c);
        if (pieces.length !== g.ids.length)
            continue;
        const head = pieces[0];
        if (!dryRun) {
            ...
            if (!merged.length)
                continue;
            for (const p of pieces)
                await storage.deleteChunk(p.id);
        }
    ...
    for (const c of all) {
        if (!c.source?.startsWith('consolidation:') || (c.origin ?? 'user') !== 'derived')
            continue;
        if (!isCut(c.content))
            continue;
        ...
        if (trimmed.length >= 40) {
            if (!dryRun)
                await storage.updateChunk(c.id, { content: trimmed });
            report.summariesTrimmed++;
        }
        else {
            if (!dryRun)
                await storage.deleteChunk(c.id);
    ...
    for (const c of all) {
        ...
        if (floor > 0 && c.importance < floor) {
            if (!dryRun)
                await storage.updateChunk(c.id, { importance: INSTRUCTION_FLOOR });
            report.floored++;
        }
    }
    return report;
